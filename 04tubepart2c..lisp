...
(setq myTube (make-object 'closed-open-tube
                          (modes 100) ;If using the envelope glissandoUPDown,MODES HAS TO BE SET LOW, modes have to be set to 30 inst. of 100 (because of the tube shrinking and expanding) otherwise it will take forever! It results in low-res sound realization but whatever
                          (length 1)
...
;make holes
;-define the hole1
(setq myHole1 (make-object 'open-open-tube
                          (modes 4)
                          (length 6.5e-3)
                          (radius0 6e-3)
                          (radius1 5e-3)
...
(setq hole1PositionOnTube (make-access myTube 0.66 'length))
...
;-define the hole2
(setq myHole2 (make-object 'open-open-tube
                          (modes 4)
                          (length 4e-3)
                          (radius0 5e-3)
                          (radius1 5e-5)
...
(setq hole2PositionOnTube (make-access myTube 0.33 'length))
